...
if (!tablaCombosDisponible($conexion)) {
    echo "<div class='alert alert-warning'>Debes ejecutar combos.sql antes de editar combos.</div>";
...
if (!$combo) {
    echo "<div class='alert alert-danger'>Combo no encontrado.</div>";
    require_once __DIR__ . '/../../includes/footer.php';
    exit;
}
...
<div class="card">
    <div class="card-header">
        <h2 class="m-0"><i class="fas fa-edit"></i> Editar Combo</h2>
    </div>
    <div class="card-body">
        <form action="guardar.php" method="POST" id="comboForm">
            <input type="hidden" name="id_combo" value="<?php echo (int)$combo['id_combo']; ?>">
            <div class="row">
                <div class="col-md-8 mb-3">
                    <label class="form-label">Nombre del combo</label>
                    <input type="text" name="nombre" class="form-control" value="<?php echo htmlspecialchars($combo['nombre']); ?>" required>
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">Precio del combo</label>
                    <input type="number" step="0.01" min="0" name="precio" class="form-control" value="<?php echo number_format($combo['precio'], 2, '.', ''); ?>" required>
                </div>
            </div>
...
<template id="comboProductoTemplate">
    <div class="row g-2 align-items-end combo-producto-row border rounded-3 p-2 bg-white">
        ...
        <div class="col-md-2">
            ...
            <input type="number" min="1" class="form-control combo-producto-cantidad" name="cantidad[]" value="1" required>
        </div>
...
<script>
    const comboProductosIniciales = <?php echo json_encode($combo['productos'], JSON_UNESCAPED_UNICODE); ?>;

    function crearFilaCombo(data = {}) {
        ...
        row.querySelector('.combo-producto-select').value = data.id_producto || '';
        row.querySelector('.combo-producto-cantidad').value = data.cantidad || 1;
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        comboProductosIniciales.forEach(producto => crearFilaCombo(producto));
